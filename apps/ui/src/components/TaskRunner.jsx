/**
 * TaskRunner — submit a goal to an agent and watch it run live.
 *
 * State flow:
 *   user types goal → clicks Run
 *   → api.streamTask opens SSE connection
 *   → each event pushed by server appends to `events` array
 *   → React re-renders EventLog with the new event
 *   → when server sends { type: "done" }, show result and stop
 */
...
export default function TaskRunner({ agents }) {
  ...
  // useRef holds the "stop" function returned by api.streamTask.
    ...
    <div className="max-w-3xl space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        {!running ? (
          ...
          <button
            onClick={handleStop}
            className="bg-red-700 hover:bg-red-600 px-5 py-2 rounded-lg text-sm
                       font-medium transition-colors"
          >
            Stop
          </button>
