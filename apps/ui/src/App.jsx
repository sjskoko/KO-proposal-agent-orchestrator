 ...
 * useState() stores data that causes the UI to re-render when it changes.
 * useEffect() runs side-effects (API calls, subscriptions) after render.
 *
 * Layout: fixed left sidebar + scrollable main area.
 */
...
export default function App() {
  // activeView controls which panel is rendered in the main area
  const [activeView, setActiveView] = useState('tasks')

  // Shared data fetched once on mount, passed down as props
  const [agents, setAgents]   = useState([])
  const [models, setModels]   = useState(null)
  const [healthy, setHealthy] = useState(null) // null = loading, true/false = result
